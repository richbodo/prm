...
| AC-4 | universal | conformant | `tests/db/test_shared_db.py::test_incompatible_schema_version_is_rejected` → live; `tests/db/test_relationships_db_version.py::test_private_mismatch_refuses_seed` → live; `tests/unit/test_daemon_diag.py::test_schema_mismatch_refuses_write_reads_continue` → live |
